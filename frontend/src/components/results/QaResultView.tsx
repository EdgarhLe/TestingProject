...
export const QaResultView: React.FC<QaResultViewProps> = ({
  response,
  isLoading,
  error,
  onRetry,
        ...
        <div className="grid grid-cols-1 gap-3 min-[360px]:grid-cols-2 sm:grid-cols-4">
          <div className="glass-panel rounded-xl border border-slate-800/80 bg-slate-950/20 p-4">
            <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-slate-500">
              <BarChart3 className="h-3.5 w-3.5 text-indigo-400" /> Results
            </div>
            <p className="mt-2 text-xl font-bold text-white">{response.total_results}</p>
          </div>
          <div className="glass-panel rounded-xl border border-slate-800/80 bg-slate-950/20 p-4">
            <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-slate-500">
              <Clock3 className="h-3.5 w-3.5 text-cyan-400" /> Query time
            </div>
            <p className="mt-2 text-xl font-bold text-white">{response.query_time_ms}ms</p>
          </div>
          <div className="glass-panel rounded-xl border border-slate-800/80 bg-slate-950/20 p-4">
            <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-slate-500">
              <Video className="h-3.5 w-3.5 text-emerald-400" /> Selected
            </div>
            <p className="mt-2 truncate text-sm font-bold text-white">{selectedResult?.video_id ?? 'None'}</p>
          </div>
          <div className="glass-panel rounded-xl border border-slate-800/80 bg-slate-950/20 p-4">
            <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-slate-500">
              <CheckCircle2 className="h-3.5 w-3.5 text-amber-400" /> Pagination
            </div>
            <p className="mt-2 text-sm font-bold text-white">{response.has_next ? 'More available' : 'Complete'}</p>
          </div>
        </div>
      )}
      ...
      <div className={selectedResult ? 'grid grid-cols-1 items-start gap-6 lg:grid-cols-12' : 'w-full'}>
        <section className={selectedResult ? 'flex min-w-0 flex-col gap-4 lg:col-span-7' : 'w-full'}>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-400">
                <BarChart3 className="h-3.5 w-3.5 text-indigo-400" /> Retrieved Q&amp;A candidates
              </h2>
              <p className="mt-1 text-xs text-slate-500">Select an answer to inspect its source segment.</p>
            </div>
            <span className="text-xs font-mono text-slate-500">{results.length} loaded</span>
          </div>
          ...
          <ResultList
            ...
            onRetry={onRetry}
            emptyMessage="No Q&A results found"
            emptyHint="Try a different scene or question."
